...
class PCAPAnalyzer:
    ...
    """
        ...
        # Ubah parameter agar menerima flag use_api
    def __init__(self, rules_dir='rules', abuseipdb_key=None, use_api=False, verbose=False):
        self.rules_dir = rules_dir
        self.abuseipdb_key = abuseipdb_key
        self.use_api = use_api # Simpan flag ini
        self.verbose = verbose
        
        self.parser = PacketParser(verbose=verbose)
        self.engine = RuleEngine(rules_dir)
        
        # PERBAIKAN: Selalu aktifkan ReputationChecker untuk Database Lokal
        # API Key bersifat opsional untuk fitur hybrid
        self.reputation = ReputationChecker(
            api_key=abuseipdb_key,
            use_api=use_api, # Teruskan flag dari user
            enable_cache=True,
            verbose=verbose
        )
    ...
    def analyze(self, pcap_file, output_dir, progress_callback=None):
        ...
        Args:
            ...
            progress_callback (function): Callback function(current, total, status)
        ...
        for packet in reader.read_packets():
            ...
            if progress_callback and stats['total_packets'] % 100 == 0:
                progress_callback(
                    stats['total_packets'],
                    stats['total_packets'],
                    f"Processing packets... {stats['total_packets']} analyzed"
                )
